...
public class Clyde extends Fantom {
    
    public Clyde(Labyrinthe labPar, Pacman pacParCible)
    {
        super(labPar, pacParCible);
        
        try {
            this.imgNotFrightened = new Image(Constantes.sCHEMIN_TEXTURE_CLYDE);
            this.imgRender = this.imgNotFrightened;
        } catch (SlickException ex) {}   
        
        posColonne  = 12*Constantes.iTAILLE_TILE_WIDTH;
        posLigne    = 11*Constantes.iTAILLE_TILE_HEIGHT-8;
    }
    
    public boolean isClydeNearPacman()
    {
        ...
        if( fPosCibleColonne < this.posColonne )
        {
            float diffColonne = this.posColonne - fPosCibleColonne;
            iColonneTest = (int)(diffColonne/Constantes.iTAILLE_TILE_WIDTH);
        }
        else
        {
            float diffColonne = fPosCibleColonne - this.posColonne;
            iColonneTest = (int)(diffColonne/Constantes.iTAILLE_TILE_WIDTH);
        }
        
        if( fPosCibleLigne < this.posLigne )
        {
            float diffLigne = this.posLigne - fPosCibleLigne;
            iLigneTest = (int)(diffLigne/Constantes.iTAILLE_TILE_HEIGHT);
        }
        else
        {
            float diffLigne = fPosCibleLigne - this.posLigne;
            iLigneTest = (int)(diffLigne/Constantes.iTAILLE_TILE_HEIGHT);
        }
        
        return (iLigneTest < 8) && (iColonneTest < 8);
    ...
    {
        float fPosCibleColonne  = Constantes.iCOLONNE_COIN_BAS_GAUCHE * Constantes.iTAILLE_TILE_WIDTH;
        float fPosCibleLigne    = Constantes.iLIGNE_COIN_BAS_GAUCHE * Constantes.iTAILLE_TILE_HEIGHT;
    ...
    public void update(GameContainer gcPar, int iParDeltaTimer)
    {
        ...
        if( this.iBehavior == Constantes.iCHASE && !isClydeNearPacman() )
        {
        ...
        else if(this.iBehavior == Constantes.iSCATTER || isClydeNearPacman())
        {
            if(this.AmIOnIntersection())
            {
                // On place blinky correctement sur l'intersection afin de simplifier au maximum les collisions
