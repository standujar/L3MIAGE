...
public class Inky extends Fantom
{    
    private Blinky blinkyCible;
    
    public Inky(Labyrinthe labPar, Pacman pacParCible, Blinky blinkyPar)
    {
        super(labPar, pacParCible);
        this.blinkyCible = blinkyPar;
        try {
            this.imgNotFrightened = new Image(Constantes.sCHEMIN_TEXTURE_INKY);
            this.imgRender = this.imgNotFrightened;
        } catch (SlickException ex) {}   
        
        posColonne  = 18*Constantes.iTAILLE_TILE_WIDTH;
     ...
     public int quelleDirectionPrendreChaseMode()
    {
        ...
        float fPosBlinkyColonne = this.blinkyCible.getPosColonne();
	float fPosBlinkyLigne = this.blinkyCible.getPosLigne();
        
        if(iDirectionPac == Constantes.iDROITE)
	{
		fPosCibleColonne=fPosCibleColonne+2*Constantes.iTAILLE_TILE_WIDTH;
	...
	{
		fPosCibleLigne=fPosCibleLigne-2*Constantes.iTAILLE_TILE_HEIGHT;
	...
	{
		fPosCibleLigne=fPosCibleLigne+2*Constantes.iTAILLE_TILE_WIDTH;
	...
	//Vector of blinky to the 2 tiles ahead of pacman *2
	if(fPosBlinkyColonne < fPosCibleColonne)
	{
		float differenceBlinkyCibleColonne = fPosCibleColonne - fPosBlinkyColonne;
		fPosCibleColonne += differenceBlinkyCibleColonne;
	...
	{
		float differenceBlinkyCibleColonne = fPosBlinkyColonne - fPosCibleColonne ;
		fPosCibleColonne -= differenceBlinkyCibleColonne;
	...
	if(fPosBlinkyLigne < fPosCibleLigne)
	{
		float differenceBlinkyCibleLigne = fPosCibleLigne - fPosBlinkyLigne;
		fPosCibleLigne += differenceBlinkyCibleLigne;
	...
	{
		float differenceBlinkyCibleLigne =  fPosBlinkyLigne - fPosCibleLigne;
		fPosCibleLigne -= differenceBlinkyCibleLigne;
    ...
    {
        float fPosCibleColonne  = Constantes.iCOLONNE_COIN_BAS_DROIT * Constantes.iTAILLE_TILE_WIDTH;
        float fPosCibleLigne    = Constantes.iLIGNE_COIN_BAS_DROIT * Constantes.iTAILLE_TILE_HEIGHT;
